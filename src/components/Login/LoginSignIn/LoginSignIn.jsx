...
class SignIn extends React.Component {
  ...
  handleValidations = (name, value) => {
    ...
    switch (name) {
      case "email":
        ...
        const userIndex = this.props.existingAccounts.findIndex(
          (item) => item.email === value
        );
        this.setState({
          passwordInQuestion: this.props.existingAccounts[userIndex].password,
        });
        break;
      case "password":
  ...
  checkErrorBeforeSave = () => {
    ...
    let isError = false;
    errorValue.emailError || errorValue.passwordError
      ? (isError = true)
      : (isError = false);
    Object.keys(authData).forEach((val) => {
      if (!authData[val].length) {
        ...
        errorValue = {
          ...errorValue,
          passwordError: "Password incorrect for this email",
        };
        isError = true;
      } else {
